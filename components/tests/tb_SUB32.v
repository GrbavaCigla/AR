...
module tb_SUB32;
    ...
    task check_case(input [31:0] test_A, input [31:0] test_B, input test_E0);
        begin
            A = test_A; B = test_B; E_0 = test_E0;
            {expected_E, expected_OUT} = A - B - E_0;
            #10;
            if (OUT === expected_OUT && E === expected_E) begin
                $display("%0dns\t  %u   %u   %b    |     %u    %b   | OK", $time, A, B, E_0, OUT, E);
            end else begin
                errors = errors + 1;
    ...
    initial begin
        $display("\n=== POKRETANJE PROVERE ZA SUB32 (8 KARAKTERISTICNIH PRIMERA) ===");
        $display("Vreme\t ULAZI (A   B  E_0) | DOBIJENO (OUT   E) | STATUS");
        $display("-----------------------------------------------------------");

        check_case(32'd0,           32'd0,           1'b0); // Primer 1
        check_case(32'd5000000,     32'd2000000,     1'b0); // Primer 2
        check_case(32'd2000000,     32'd5000000,     1'b0); // Primer 3 (izaziva E=1)
        check_case(32'h8000_0000,   32'd1,           1'b0); // Primer 4
        check_case(32'd0,           32'd0,           1'b1); // Primer 5
        check_case(32'hFFFF_FFFF,   32'hFFFF_FFFF,   1'b0); // Primer 6
        check_case(32'hFFFF_FFFF,   32'd0,           1'b1); // Primer 7
        check_case(32'd0,           32'hFFFF_FFFF,   1'b1); // Primer 8
